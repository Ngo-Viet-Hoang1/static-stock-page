    ...
    <section className="py-24 bg-white dark:bg-dark-950 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="container mx-auto px-6 text-center relative z-10">
        <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4">
          Trải Nghiệm <span className="text-yellow-600 dark:text-gold-400">Thực Tế</span>
        </h2>
        <p className="text-slate-600 dark:text-gray-400 mb-12 max-w-xl mx-auto">Xem giới thiệu ngắn về khóa học và phương pháp đầu tư độc quyền của chúng tôi.</p>
        ...
        <div className="relative max-w-5xl mx-auto p-4 bg-white/20 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]">
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-inner bg-black group">
            ...
            <div className={`absolute inset-0 flex items-center justify-center bg-black/40 transition-opacity duration-300 ${isPlaying ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}>
              ...
              >
                {isPlaying ? <Pause className="text-white fill-current" size={32} /> : <Play className="text-white fill-current ml-2" size={32} />}
              </button>
            </div>

            <div className="absolute bottom-6 right-6 z-10">
               <button onClick={toggleMute} className="p-3 bg-black/40 backdrop-blur-md border border-white/10 rounded-full text-white hover:bg-white/20 transition-colors">
        ...
        <p className="mt-6 text-sm text-slate-500 dark:text-gray-500 italic">* Video có phụ đề tiếng Việt. Bấm vào icon âm thanh để nghe Voice-over.</p>
      </div>
    </section>
  );
};
